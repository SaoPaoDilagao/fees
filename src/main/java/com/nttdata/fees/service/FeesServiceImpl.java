...
public class FeesServiceImpl implements FeesService {
	...
	public Flux<Fee> deleteFees(String idTransaction) {
		...
		return listByIdTransaction(idTransaction).map(item -> {
			ids.add(item.getId());
			return item;
		}).flatMap(doc -> {
	...
	private BigDecimal calculateRateAmount(FeeRequest request) {
		...
		BigDecimal singleAmount = request.getAmount().divide(new BigDecimal(request.getNumberOfFees()), 2,
				RoundingMode.HALF_DOWN);
		BigDecimal singleInterest = singleAmount.multiply(request.getPercentageInterestRate())
	...
	public Mono<Fee> updateFee(String id) {
		return findFeeById(id).map(item -> {
